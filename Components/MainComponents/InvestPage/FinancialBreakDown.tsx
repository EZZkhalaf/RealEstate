...
const FinancialBreakDown: React.FC<{
  ...
  totalExpensest: number;
  totalExpenses: number;
  netCashFlow: number;
}> = ({ grossAnnualRent, totalExpensest, totalExpenses, netCashFlow }) => {
  return (
    <div className="flex flex-col gap-2 rounded-lg shadow-2xl py-3 px-6">
      ...
      <div className="flex flex-col gap-2 mt-3">
        <div className="flex justify-between  border-b border-gray-200">
          ...
          <p className="font-semibold text-sm ">{grossAnnualRent}</p>
        </div>
        <div className="flex justify-between border-b border-gray-200">
        ...
        <div className="flex justify-between border-b border-gray-200">
          ...
          <p className="font-semibold text-red-700 text-sm">₹{totalExpenses}</p>
        </div>
        <GrayLine />
        <div className="flex justify-between ">
          <p className="font-bold text-md">Net Cash Flow</p>
          <p className="font-semibold text-red-900 text-md">₹{netCashFlow}</p>
        </div>
      </div>
    </div>
  );
